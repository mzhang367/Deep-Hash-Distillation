...
def Evaluate_mAP(device, gallery_codes, query_codes, gallery_labels, query_labels, Top_N=None):
    ...
    for i in range(num_query):
        retrieval = (query_labels[i, :] @ gallery_labels.t() > 0).float()  # identify true/false items
...
def DoRetrieval(device, net, Img_dir, Gallery_dir, Query_dir, NB_CLS, Top_N, args):
    Gallery_set = Loader(Img_dir, Gallery_dir, NB_CLS, folder='train' if args.dataset=='imagenet' else None)
    Gallery_loader = T.utils.data.DataLoader(Gallery_set, batch_size=args.batch_size, num_workers=args.num_workers)

    Query_set = Loader(Img_dir, Query_dir, NB_CLS, folder='val' if args.dataset=='imagenet' else None)
    ...
    with T.no_grad():
        for i, data in enumerate(Gallery_loader, 0):
            ...
            if gallery_y_batch.dim() == 1: # alias for ndim, i.e. a batch of single labels w/o onehot
